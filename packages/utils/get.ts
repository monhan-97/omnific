...
import type { PropertyPath } from './@types/PropertyPath';
import { isNil } from './isNil';
import { isNumber } from './isNumber';
import { normalizeKey, toKey } from './_internal/toKey';
...
export function get<TObject, TPath extends string, TDefault = GetFieldType<TObject, TPath>>(
  ...
  path: TPath,
  defaultValue: TDefault,
): Exclude<GetFieldType<TObject, TPath>, undefined> | TDefault;
...
export function get(
  ...
  if (isString(path)) {
    ...
    if (isUndefined(result)) {
      return isDeepKey(path) ? getWithPath(object, toPath(path), defaultValue) : defaultValue;
    }
    return result;
  }
...
function getWithPath(object: any, path: readonly PropertyKey[], defaultValue?: any): any {
  ...
  for (const element of path) {
    if (isNil(current) || isUnsafeProperty(element)) {
      return defaultValue;
    }
...
function getDirect(object: any, key: PropertyKey, defaultValue?: any): any {
  if (isUnsafeProperty(key)) return defaultValue;
